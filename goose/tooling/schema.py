...
def _get_type_name(annotation: Any) -> str:
    ...
    if origin is not None:
        ...
        if origin is list:
            if args:
                return f"array[{_get_type_name(args[0])}]"
            return "array"
        if origin is dict:
            return "object"
...
def list_tool_schemas(tools: list[Callable]) -> list[ToolSchema]:
...
def get_tool_by_name(tools: list[Callable], name: str) -> Callable | None:
